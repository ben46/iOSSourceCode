...
public:
    OSSpinLock slock;// 保证原子操作的自旋锁, 为了防止竞争选择的自旋锁
    RefcountMap refcnts;     // 引用计数的 hash 表, 协助对象的 isa 指针的 extra_rc 共同引用计数的变量
...
    // 下面指向的操作会改变旧值
 retry:
    ...
    lock1 = &newTable->slock; // 保证原子操作的自旋锁
#if SIDE_TABLE_STRIPE > 1
...
    // 避免线程冲突重处理
    ...
        // ③ 分配新值
        ...
        // newTable->weak_table 存储了某个对象相关的的所有的弱引用信息
        // 比如, 保存了所有指向指定对象的 weak 指针, 存储空间, 参与判断引用计数辅助量, hash key 最大偏移值
    }
    // Do not set *location anywhere else. That would introduce a race.
    *location = newObj; // 接下来让弱引用指针指向新的对象
...
id
objc_initWeak(id *addr, id val) // objc_initWeak函数有一个前提条件：就是object必须是一个没有被注册为__weak对象的有效指针。
...
void 
objc_clear_deallocating(id obj) //当weak引用指向的对象被释放时, 这是调用的最后一步
{
    assert(obj);
    assert(!UseGC);
    
    // 首先取出对象对应的SideTable实例
    SideTable *table = SideTable::tableForPointer(obj);
    ...
    OSSpinLockLock(&table->slock);
    if (seen_weak_refs) { // 这个对象有关联的弱引用
        arr_clear_deallocating(&table->weak_table, obj);
        // 调用arr_clear_deallocating来清除对象的弱引用信息
        // 首先是找出对象对应的weak_entry_t链表
        // 然后挨个将弱引用置为nil。
        // 最后清理对象的记录
    }
